...
y = [1, 0, 1, 0]  # example labels (1=positive, 0=negative)
...
vectorizer = pkl.load(open('vectorizerer.pkl', 'rb'))  # will work if fitted
clt = pkl.load(open('nlp_model.pkl', 'rb'))

# When predicting:
text = "I love this movie"
X = vectorizer.transform([text])  # will now work
prediction = clt.predict(X)
...
def get(movie_name, userId):
    print(f"Request received for movie: {movie_name}, userId: {userId}", strict_slashes=False)
...
# @app.route('/review/<movieId>/<path:review>/<string:userId>', methods=["GET"])
...
# ==============================GPT =======================
#API CALL IN FRONTEND IN FORMAT /rate/84/1.5/YQ67YHFYTYUHF67686TR7T7T7868
@app.route('/rate/<movieId>/<rate>/<string:userId>', methods=["GET"])
def rate_movie(movieId, rate, userId):
    import csv
    try:
        rate = float(rate)
