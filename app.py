...
def train_models():
    ...
    reviews = pd.read_csv("reviews.csv")  # assumes 'content' + 'label' cols
...
def load_models():
    ...
    try:
        ...
        with open('clf_vectorizer.pkl', 'rb') as f:
        ...
        print("⚠️ Error loading models, train first:", e)
...
def getrating(title):
    ...
    data = getreview(title)
    for i in data['results']:
        pred = clf.predict(clf_vectorizer.transform([i['content']]))
        movie_review.append({"review": i['content'], "rating": "Good" if pred[0] == 1 else "Bad"})
    return movie_review
